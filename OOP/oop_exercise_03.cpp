...
class Trapezoid : public Figure {       //равнобедренная трапеция
public:
    Trapezoid() {};
...
double lenght(Point a, Point b) {       //вычисление длины стороны
    return sqrt((b.x - a.x)*(b.x - a.x) + ((b.y - a.y)*(b.y - a.y)));
}

int CorrectInput(Point& a, Point& b, Point& c, Point& d, int fig){      //проверка коректности введенной фигуры
...
int main() {
    ...
    for (int i = 0; i < count_sq; ++i) {    //ввод квадратов (корректный ввод координат: по часовой стрелке)
        cout << "Enter squares:" << '\n';
        cin >> a >> b >> c >> d;
    ...
    for (int i = 0; i < count_rest; ++i) {   //ввод прямоугольников (корректный ввод координат: по часовой стрелке)
    ...
    for (int i = 0; i < count_trap; ++i) {  //ввод равнобедренных трапеций (корректный ввод координат: по часовой стрелке)
