// Ћагуткина ћари€, ћ8ќ-206Ѕ-19
//¬ариант 10: фигуры квадрат, пр€моугольник, трапеци€
//–азработать шаблоны классов согласно варианту задани€.  
//ѕараметром шаблона должен €вл€тьс€ скал€рный тип данных задающий тип данных дл€ оси координат. 
// лассы должны иметь только публичные пол€. ¬ классах не должно быть методов, только пол€. 
//‘игуры €вл€ютс€ фигурами вращени€ (равнобедренными), за исключением трапеции и пр€моугольника. 
//ƒл€ хранени€ координат фигур необходимо использовать шаблон  std::pair.
...
int CorrectInput(pair<T, T>& a, pair<T, T>& b, pair<T, T>& c, pair<T, T>& d, int f) {      //проверка коректности введенной фигуры
    ...
    //проверка коректности пр€моугольника: равенсво противоположных сторон и диагоналей
    if (f == 2) {
